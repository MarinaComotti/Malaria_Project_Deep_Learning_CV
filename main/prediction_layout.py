...
def create_progress_bar():
    progress_text = "Generando predicción..."
...
def show_prediction(image, real_class):
    ...
    if prediction_value < 0.5:
        st.markdown(
            '<font size="6">Resultado: <b>Parasitized</b></font>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<font size="6">Resultado: <b>Uninfected</b></font>',
            unsafe_allow_html=True,
        )
    if real_class is not None:
        st.markdown(
            f'<font size="4">Clasificación real: <b>{real_class}</b></font>',
            unsafe_allow_html=True,
        )
...
def create_columns(image, selected_image_name, image_class=None):
    ...
    with col2:
        if st.button("Probar en el modelo", key=image_class):
            create_progress_bar()
